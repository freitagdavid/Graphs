...
def dft():
    ...
    current_traversal = []
    populate_room(traversal_graph)
    q = deque([traversal_graph[0]])
    while q and loops < 2000 and len(traversal_graph) < len(roomGraph):
        loops += 1
        unexplored = []
        exits = player.currentRoom.getExits()
        for i in exits:
            if traversal_graph[player.currentRoom.id][i] == '?':
                unexplored.append(i)
        if unexplored:
            direction = unexplored.pop()
            current_traversal.append(direction)
            walk(direction, traversal_graph)
        else:
            # print(current_traversal)
            direction = invert_dir(current_traversal.pop())
            walk(direction, traversal_graph)
        if dbg:
            terminal_size = get_terminal_size()
            print('\n')
            print(f' loop {loops} '.center(terminal_size.columns, '#'))
            print('\n')
            print('Traversal Graph:', traversal_graph)
            print('\n')
            print('Current Room', player.currentRoom)
            print('\n')
            print('Traversal Graph for Room:',
                  traversal_graph[player.currentRoom.id])
            print('\n')
